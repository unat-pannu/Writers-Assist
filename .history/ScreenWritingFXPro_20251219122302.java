...
public class ScreenWritingFXPro extends Application {
    ...
    private void loadProjects() {
        ...
        try (Connection con = DBUtil.getConnection()) {
            ...
            while (rs.next()) {
                ...
                PreparedStatement ps = con.prepareStatement("SELECT * FROM Notes WHERE project_id=?");
    ...
    private void updateBoard() {
        ...
        try (Connection con = DBUtil.getConnection()) {
            Statement st = con.createStatement();
            ResultSet rs = st.executeQuery("SELECT n.note_id, n.title, p.title AS project FROM Notes n JOIN Project p ON n.project_id=p.project_id");
        while (rs.next()) {
    ...
    private void createNote() {
        ...
        dialog.showAndWait().ifPresent(title -> {
            try (Connection con = DBUtil.getConnection()) {
                PreparedStatement ps = con.prepareStatement("INSERT INTO Notes(title, content, project_id) VALUES(?,?,?)");
    ...
    private void deleteSelected() {
        ...
        try (Connection con = DBUtil.getConnection()) {
            if (selected.getParent() == treeView.getRoot()) {
                ...
                // Delete note
                PreparedStatement ps = con.prepareStatement("DELETE FROM Notes WHERE title=?");
