...
public class ScreenWritingFXPro extends Application {
    ...
    public void start(Stage stage) {
        ...
        // Ensure Widgets table supports image BLOB + sizes
        ensureWidgetsSchema();
    ...
    private void ensureWidgetsSchema() {
        try (Connection con = getConnection(); Statement st = con.createStatement()) {
            ...
            // Add columns if existing table was older (ignore "duplicate column" errors)
    ...
    private void saveWidget(String type, String content, File imageFileOrNull) {
        try (Connection con = getConnection()) {
            ...
            if ("IMAGE_BLOB".equals(type) && imageFileOrNull != null) {
                try (FileInputStream fis = new FileInputStream(imageFileOrNull)) {
                    ps.setBinaryStream(7, fis, (int) imageFileOrNull.length());
    ...
    private Pane buildWidget(int id, String type, String content, double w, double h, Blob blob, String imageName) {
        ...
        // IMPORTANT FIX: Node, not Region (WebView is Node but not Region)
        Node body = switch (type) {
            ...
            case "IMAGE_BLOB" -> {
                ...
                img.fitHeightProperty().bind(card.heightProperty().subtract(80));
            ...
            default -> {
                ...
                // Wrap WebView in a Region so resizing is well-behaved inside VBox
                StackPane webWrap = new StackPane(web);
                webWrap.setPadding(new Insets(0));

                web.prefWidthProperty().bind(card.widthProperty().subtract(20));
                web.prefHeightProperty().bind(card.heightProperty().subtract(60));

                VBox.setVgrow(webWrap, Priority.ALWAYS);
                yield webWrap;
            }
        };
    ...
    private void deleteWidget(int id) {
        try (Connection con = getConnection()) {
            PreparedStatement ps = con.prepareStatement("DELETE FROM Widgets WHERE Widget_id=?");
            ps.setInt(1, id);
            ps.executeUpdate();
            loadWidgets();
            loadJoinTable();
    ...
    // =========================================================
    //  JOIN OPERATION (Project JOIN Widgets)
    // =========================================================
